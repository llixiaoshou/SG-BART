...
from modeling_bart_dep import BartForConditionalGeneration, shift_tokens_right
from tensorboardX import SummaryWriter

class CoNTGenerator(nn.Module):

    def __init__(self, model_name, pad_id, args, model_args):
        super(CoNTGenerator, self).__init__()
        self.model_name=model_name
        # self.model_args=self.model_args[ASP_config]
        self.generator = BartForConditionalGeneration.from_pretrained(self.model_name,**model_args)
        # if self.PTM == "pegasus":
        ...
        self.args = args
        self.writer=SummaryWriter('./graph')
    ...
    @torch.no_grad()
    def sample_from_model(self, src_inp, src_pad_mask,graph_undir,edge_matrix_undir):
    ...
    @torch.no_grad()
    def generate(self, input_ids, attention_mask,graph_undir ,edge_matrix_undir,classification, target_outp,args):
        self.generator.eval()
    ...
    def forward(self, src_inp,src_pad_mask, graph_undir,edge_matrix_undir ,classification,target_outp):
        """
        cos_score distance of hypothesis to source
        """
        outputs = self.generator(src_inp, src_pad_mask, graph_undir, edge_matrix_undir, classification,labels=target_outp,
                                 output_hidden_states=True, return_dict=True)
        # outputs = self.generator(src_inp, src_pad_mask, graph_undir, edge_matrix_undir, labels=target_outp,
        #                          output_hidden_states=True,return_dict=True)
        # print(outputs)
        ...
        # batch_size = src_inp.size(0)
        # # src_pad_mask = ~(src_inp == self.pad_id)
        # encoder_hidden_states = encoder(src_inp, src_pad_mask,graph_undir,edge_matrix_undir)['last_hidden_state']
        ...
        target_inp = shift_tokens_right(
            target_outp, self.generator.config.pad_token_id, self.generator.config.decoder_start_token_id
        )
        batch_size = src_inp.size(0)

        ###### Contrastive loss ######
        cand_ids = self.sample_from_model(src_inp, src_pad_mask,graph_undir,edge_matrix_undir)  # batch x beam_size x seq_len
        ...
        #####################################
        if self.generator.training is not True:
            graph_label_enc = torch.zeros(encoder_feature.shape[0], 1)
            graph_label_dec = torch.ones(encoder_feature.shape[0], decoder_feature.shape[1])
            graph_label_dec[:,1:]=2
            graph_label=torch.cat([graph_label_enc,graph_label_dec],dim=1).view(-1)
            self.writer.add_embedding(torch.cat([encoder_feature.unsqueeze(1),decoder_feature],dim=1).view(-1,768),graph_label)
            ####################################
        cos_distance = torch.cosine_similarity(encoder_feature.unsqueeze(1), decoder_feature,
                                               dim=-1)  # batch x samle_num
        cl_loss = self.ranking_loss(cos_distance, sampled_bleu_distance)
        # print('++++++++++++++++++++++')
        return {'loss': nll_loss + cl_loss, "cl_loss": cl_loss,"sim":actual_distance}
