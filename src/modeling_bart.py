...
class BartModel(BartPreTrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.zuo_predict=nn.Linear(self.dim,1)
        self.zuo_embed = nn.Parameter(torch.randn(2, self.dim))
    ...
    def forward(
        ...
        if classification is not None:
            ...
            envi_loss=self.focalloss(envi_prob,classification)
            ...
            #     teach_force = (classification.unsqueeze(-1) - envi_prob).detach() + envi_prob
            #     # print(envi.shape)
            #     new_envi = teach_force
            envi_feat = new_envi*self.zuo_embed[0].unsqueeze(0)+(1-new_envi)*self.zuo_embed[1].unsqueeze(0)
            # print(envi_feat.shape)
            envi_feat = envi_feat.view(bsz,-1,self.dim)
...
class BartForConditionalGeneration(BartPreTrainedModel):
    ...
    def forward(
        ...
        # print('envi_loss',envi_loss.shape)
        # masked_lm_loss = None
        if labels is not None:
