...
def build_encoder(opt, answer=False, graph=False):
    ...
    if graph:
        ...
        options = {'n_vocab':opt.src_vocab_size, 'd_word_vec':opt.d_word_vec, 'd_model':opt.d_seq_enc_model,
                   ...
                   'feat_vocab':feat_vocab, 'd_feat_vec':opt.d_feat_vec, 'dropout':opt.dropout,'ans_n_vocab':opt.src_vocab_size}
...
def build_model(opt, device, separate=-1, checkpoint=None):
    ...
    encoder_transformer = EncoderTransformer(opt.d_seq_enc_model, opt.d_feat_vec,opt.node_feat_vocab, d_k=opt.d_k,device=device)
    graph_encoder = build_encoder(opt, graph=True)
    Concat_Attention = ConcatAttention(512, 512, 64)
    # transformer_encoder=Transformer_encoder(opt)

    Ai_transformer = Transformer(device=device)


    if opt.d_seq_enc_model != opt.d_graph_enc_model:
        # encoder_transformer.activate = nn.Sequential(
        ...
        # encoder_transformer.activate=nn.Tanh()

    decoder_transformer = DecoderTransformer(opt.layer_attn, device=device)
    decoder = build_decoder(opt, device)

    model = UnifiedModel(opt.training_mode, seq_encoder, graph_encoder, encoder_transformer, Ai_transformer,
                         decoder, decoder_transformer,Concat_Attention)

    m=opt.d_dec_model
    # print('1+',m)
    n=opt.maxout_pool_size
    # print('2+',n)
    b=opt.tgt_vocab_size
    # print('3_',b)
    # model.generator = nn.Linear(opt.d_dec_model // opt.maxout_pool_size, opt.tgt_vocab_size, bias=False)
    #同样的,这里也改成512,512了
    model.generator = nn.Linear(opt.d_dec_model, opt.tgt_vocab_size, bias=False)
    model.w=nn.Linear(512,opt.tgt_vocab_size)
    model.projection=nn.Sequential(nn.Linear(512,  512),
                                        nn.ReLU()).cuda()
    model.res_class = nn.Linear(1024, 1).cuda()
