...
class SupervisedTrainer(object):
    ...
    def eval_step(self, device, epoch):
        ...
        with torch.no_grad():
            for idx in tqdm(range(len(self.validation_data)), mininterval=2, desc='  - (Validation) ', leave=False):
                ...
                inputs, max_length, golds, copy = preprocess_batch(batch, self.opt.edge_vocab_size,
                ...
                input_ids, attention_mask = inputs['seq-encoder']["src_seq"].to(graph_undir.device), inputs['seq-encoder']["src_mask"].to(
                    graph_undir.device)

                rst = self.model(input_ids,attention_mask=attention_mask,
                ...
                if self.opt.training_mode != 'classify':
                    ...
                    #
                    # print('m',m2)
                    loss_input['generation']['pred'] = rst.logits[:,:max_tgt_seq_length-2,:]
    ...
    def train_epoch(self, device, epoch):
        ...
        for idx in tqdm(range(len(self.training_data)), mininterval=2, desc='  - (Training)   ', leave=False):
            ...
            ##### ==================== prepare data ==================== #####
            inputs, max_length, golds, copy = preprocess_batch(batch, self.opt.edge_vocab_size, sparse=self.opt.sparse,
            ...
            graph_undir,edge_matrix_undir = inputs['graph-encoder']['graph_undir'],inputs['graph-encoder']['edge_matrix_undir']
            input_ids, attention_mask = inputs['seq-encoder']["src_seq"].to(graph_undir.device), inputs['seq-encoder']["src_mask"].to(graph_undir.device)
            from transformers import AutoTokenizer
            tokenizer = AutoTokenizer.from_pretrained('../pre_model/bart-base')
            # print(inputs['decoder']['src_seq'][0])
            # print(inputs['decoder']['src_seq'].shape)
            # print(tokenizer.decode(inputs['decoder']['src_seq'][0].detach().cpu().numpy()))
            # # assert 0==1
            # print(tokenizer.decode(inputs['decoder']['tgt_seq'][0].detach().cpu().numpy()))
            # print(tokenizer.decode(inputs['decoder']['ans_seq'][0].detach().cpu().numpy()))
            # print(tokenizer.decode(inputs['seq-encoder']['ans_seq'][0].detach().cpu().numpy()))
            # # print(tokenizer.decode(inputs['seq-encoder']['ans_seq'][0].detach().cpu().numpy()))
            # assert 0 == 1
            ...
            # print('attention_mask',attention_mask.shape)
            # print('decoder_input_ids',inputs['decoder']['tgt_seq'].shape)
            # print('decoder_attention_mask',inputs['decoder']['tgt_seq_mask'].shape)
            ...
            if self.opt.training_mode != 'classify':
                # loss_input['generation']['m2'] = rst['dec_output1']
                loss_input['generation']['pred'] = rst.logits[ :,:max_tgt_seq_length-2,:]
                c=rst.logits[ :,:max_tgt_seq_length-2,:]
                print('c',c.shape)
                ...
                # (4,3,768) -> (4,2,768)
                loss_input['generation']['gold'] = golds[0]
                print('gold[0].shape',golds[0].shape)
